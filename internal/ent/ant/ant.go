...
const (
	// Label holds the string label denoting the ant type in the database.
	Label = "ant"
	...
	EdgeTask = "task"
	// Table holds the table name of the ant in the database.
	Table = "ant"
...
// Columns holds all SQL columns for ant fields.
var Columns = []string{
	FieldID,
	FieldCreatedAt,
	FieldUpdatedAt,
...
func OsValidator(o shared.AntOs) error {
	switch o.String() {
	case "unknown", "linux", "windows", "macos":
		return nil
	default:
		return fmt.Errorf("ant: invalid enum value for os field: %q", o)
	}
}

// ArchValidator is a validator for the "arch" field enum values. It is called by the builders before save.
func ArchValidator(a shared.AntArch) error {
	switch a.String() {
	case "unknown", "x86", "x64", "arm32", "arm64":
		return nil
	default:
		return fmt.Errorf("ant: invalid enum value for arch field: %q", a)
	}
}

// OrderOption defines the ordering options for the Ant queries.
...
func ByCommandCount(opts ...sql.OrderTermOption) OrderOption {
	return func(s *sql.Selector) {
		sqlgraph.OrderByNeighborsCount(s, newCommandStep(), opts...)
	}
}
...
func ByCommand(term sql.OrderTerm, terms ...sql.OrderTerm) OrderOption {
	return func(s *sql.Selector) {
		sqlgraph.OrderByNeighborTerms(s, newCommandStep(), append([]sql.OrderTerm{term}, terms...)...)
	}
}
