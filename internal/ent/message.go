...
func (*Message) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case message.FieldType:
			values[i] = new(shared.TaskMessage)
		case message.FieldID, message.FieldCommandID:
	...
	for i := range columns {
		...
		case message.FieldCommandID:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				...
				m.CommandID = value.Int64
			}
		case message.FieldType:
			if value, ok := values[i].(*shared.TaskMessage); !ok {
...
func (m *Message) QueryCommand() *CommandQuery {
	return NewMessageClient(m.config).QueryCommand(m)
}
...
func (m *Message) String() string {
	...
	builder.WriteString(fmt.Sprintf("%v", m.CommandID))
	builder.WriteString(", ")
