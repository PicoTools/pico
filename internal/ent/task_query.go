...
func (tq *TaskQuery) QueryCommand() *CommandQuery {
	query := (&CommandClient{config: tq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, true, task.CommandTable, task.CommandColumn),
		)
...
func (tq *TaskQuery) QueryBlobberArgs() *BlobberQuery {
	query := (&BlobberClient{config: tq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// WithBlobberArgs tells the query-builder to eager-load the nodes that are connected to
// the "blobber_args" edge. The optional arguments are used to configure the query builder of the edge.
func (tq *TaskQuery) WithBlobberArgs(opts ...func(*BlobberQuery)) *TaskQuery {
	query := (&BlobberClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withBlobberArgs = query
	return tq
}

// WithBlobberOutput tells the query-builder to eager-load the nodes that are connected to
// the "blobber_output" edge. The optional arguments are used to configure the query builder of the edge.
func (tq *TaskQuery) WithBlobberOutput(opts ...func(*BlobberQuery)) *TaskQuery {
	query := (&BlobberClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withBlobberOutput = query
	return tq
}
...
func (tq *TaskQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Task, error) {
	...
	if query := tq.withCommand; query != nil {
		if err := tq.loadCommand(ctx, query, nodes, nil,
			func(n *Task, e *Command) { n.Edges.Command = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := tq.withBlobberArgs; query != nil {
		if err := tq.loadBlobberArgs(ctx, query, nodes, nil,
			func(n *Task, e *Blobber) { n.Edges.BlobberArgs = e }); err != nil {
			return nil, err
		}
	}
	if query := tq.withBlobberOutput; query != nil {
		if err := tq.loadBlobberOutput(ctx, query, nodes, nil,
			func(n *Task, e *Blobber) { n.Edges.BlobberOutput = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TaskQuery) loadCommand(ctx context.Context, query *CommandQuery, nodes []*Task, init func(*Task), assign func(*Task, *Command)) error {
...
func (tq *TaskQuery) loadBlobberArgs(ctx context.Context, query *BlobberQuery, nodes []*Task, init func(*Task), assign func(*Task, *Blobber)) error {
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "args_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (tq *TaskQuery) loadBlobberOutput(ctx context.Context, query *BlobberQuery, nodes []*Task, init func(*Task), assign func(*Task, *Blobber)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Task)
	for i := range nodes {
		fk := nodes[i].OutputID
...
func (tq *TaskQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := tq.ctx.Fields; len(fields) > 0 {
		...
		if tq.withBlobberOutput != nil {
			_spec.Node.AddColumnOnce(task.FieldOutputID)
		}
	}
