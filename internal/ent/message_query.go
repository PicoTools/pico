...
func (mq *MessageQuery) QueryCommand() *CommandQuery {
	query := (&CommandClient{config: mq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, true, message.CommandTable, message.CommandColumn),
		)
...
func (mq *MessageQuery) Clone() *MessageQuery {
	...
	return &MessageQuery{
		...
		withCommand: mq.withCommand.Clone(),
...
func (mq *MessageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Message, error) {
	...
	if query := mq.withCommand; query != nil {
		if err := mq.loadCommand(ctx, query, nodes, nil,
			func(n *Message, e *Command) { n.Edges.Command = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (mq *MessageQuery) loadCommand(ctx context.Context, query *CommandQuery, nodes []*Message, init func(*Message), assign func(*Message, *Command)) error {
...
func (mq *MessageQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := mq.ctx.Fields; len(fields) > 0 {
		...
		if mq.withCommand != nil {
			_spec.Node.AddColumnOnce(message.FieldCommandID)
		}
	}
