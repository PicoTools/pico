...
func (mc *MessageCreate) check() error {
	if _, ok := mc.mutation.CommandID(); !ok {
		return &ValidationError{Name: "command_id", err: errors.New(`ent: missing required field "Message.command_id"`)}
	}
	...
	if len(mc.mutation.CommandIDs()) == 0 {
		return &ValidationError{Name: "command", err: errors.New(`ent: missing required edge "Message.command"`)}
	}
	return nil
}
...
func (mc *MessageCreate) createSpec() (*Message, *sqlgraph.CreateSpec) {
	...
	if nodes := mc.mutation.CommandIDs(); len(nodes) > 0 {
