...
func (cq *ChatQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Chat, error) {
	...
	if query := cq.withOperator; query != nil {
		if err := cq.loadOperator(ctx, query, nodes, nil,
			func(n *Chat, e *Operator) { n.Edges.Operator = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *ChatQuery) loadOperator(ctx context.Context, query *OperatorQuery, nodes []*Chat, init func(*Chat), assign func(*Chat, *Operator)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*Chat)
	for i := range nodes {
		fk := nodes[i].AuthorID
...
func (cq *ChatQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := cq.ctx.Fields; len(fields) > 0 {
		...
		if cq.withOperator != nil {
			_spec.Node.AddColumnOnce(chat.FieldAuthorID)
		}
	}
