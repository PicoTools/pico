...
func (f CommandFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.CommandQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.CommandQuery", q)
}
...
func (f TraverseCommand) Traverse(ctx context.Context, q ent.Query) error {
	...
	return fmt.Errorf("unexpected query type %T. expect *ent.CommandQuery", q)
}
...
func NewQuery(q ent.Query) (Query, error) {
	...
	case *ent.CommandQuery:
		return &query[*ent.CommandQuery, predicate.Command, command.OrderOption]{typ: ent.TypeCommand, tq: q}, nil
