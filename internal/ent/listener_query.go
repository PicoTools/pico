...
func (lq *ListenerQuery) loadAgent(ctx context.Context, query *AgentQuery, nodes []*Listener, init func(*Listener), assign func(*Listener, *Agent)) error {
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "listener_id" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (lq *ListenerQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(listener.Table, listener.Columns, sqlgraph.NewFieldSpec(listener.FieldID, field.TypeInt64))
	_spec.From = lq.sql
