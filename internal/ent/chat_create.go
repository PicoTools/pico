...
func (cc *ChatCreate) defaults() {
	...
	if _, ok := cc.mutation.IsServer(); !ok {
		v := chat.DefaultIsServer
		cc.mutation.SetIsServer(v)
	}
}
...
func (cc *ChatCreate) check() error {
	...
	if _, ok := cc.mutation.IsServer(); !ok {
		return &ValidationError{Name: "is_server", err: errors.New(`ent: missing required field "Chat.is_server"`)}
	}
	return nil
}
...
func (cc *ChatCreate) createSpec() (*Chat, *sqlgraph.CreateSpec) {
	...
	if value, ok := cc.mutation.IsServer(); ok {
